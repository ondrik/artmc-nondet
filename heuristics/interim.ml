...
(****************************************************************************************)
												(*union*)
(****************************************************************************************)
let union ?(marks = ("a","b")) a b = 
	if a.a_alphabet <> b.a_alphabet then failwith"Interim.union";
	let na = Array.length a.a_states in

	let mark_and_shift mark shift states = 
		Array.map (fun {s_name=nm; s_index=i; s_final=fin} -> {s_name=mark^nm; s_index=i+shift; s_final=fin}) 
		states
	in

	let new_states = 
		Array.append (mark_and_shift (fst marks) 0 a.a_states) 
		             (mark_and_shift (snd marks) (Array.length a.a_states) b.a_states)
	in
...
(*sorts lists and divides into equivalence classes -- (lists in reversed order!!)*)
